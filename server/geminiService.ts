...
export function buildSystemPrompt(clinicName: string, pacienteCadastrado?: { name: string; totalAppointments: number }): string {
  const today = new Date();
  const dateStr = today.toISOString().split("T")[0];

  const contextoPaciente = pacienteCadastrado
    ? `\nCONTEXTO DESTE PACIENTE: Este número de WhatsApp já é de um paciente CADASTRADO chamado "${pacienteCadastrado.name}" (já veio à clínica ${pacienteCadastrado.totalAppointments}x anteriormente). NÃO pergunte o nome dele — cumprimente-o diretamente pelo nome já cadastrado. Use exatamente esse nome ao chamar 'criar_agendamento'.\n`
    : `\nCONTEXTO DESTE PACIENTE: Este número de WhatsApp ainda NÃO está cadastrado na clínica. Pergunte o nome completo antes de agendar. NUNCA peça CPF, RG ou outros documentos pelo WhatsApp — o cadastro completo (CPF, data de nascimento, etc.) é feito presencialmente na recepção no dia da consulta, por segurança dos dados do paciente.\n`;
...
1. NUNCA chame 'criar_agendamento' sem antes ter chamado 'consultar_horarios_disponiveis' NESTA MESMA CONVERSA e ter o paciente escolhido um dos horários REAIS retornados por essa ferramenta. É proibido inventar, estimar ou "chutar" data/hora — use exclusivamente os valores exatos (campo "data" e "hora") que a ferramenta 'consultar_horarios_disponiveis' devolveu. O sistema VERIFICA isso automaticamente e recusa o agendamento mesmo que a data/hora "chutada" coincida por acaso com um horário real — não existe atalho.
...
export async function processWhatsAppMessage(
  ...
  const cadastrado = clinicDb.buscarPacientePorTelefone(patientPhone);
  const systemPrompt = buildSystemPrompt(
    clinicConfig.clinicName,
    cadastrado ? { name: cadastrado.name, totalAppointments: cadastrado.totalAppointments || 0 } : undefined
  );
    ...
    if (functionCalls && functionCalls.length > 0) {
      ...
      for (const call of functionCalls) {
        ...
        if (name === "consultar_horarios_disponiveis") {
          result = clinicDb.consultarHorarios(args.servico, args.data_preferida, args.medico, patientPhone);
...
async function simulateLocalProcessing(
  ...
  // Rule 6: Consultation of available slots (Search)
    ...
    const res = clinicDb.consultarHorarios(srv, lower.includes("amanha") || lower.includes("amanhã") ? "amanhã" : undefined, undefined, patientPhone);
