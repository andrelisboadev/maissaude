...
class ClinicDatabase {
  ...
  /**
   * Guarda, por telefone do paciente, quais horários foram REALMENTE oferecidos numa consulta
   * recente — usado como trava em criarAgendamento para impedir que a IA "acerte por sorte"
   * um horário existente sem nunca ter mostrado as opções reais ao paciente.
   */
  private offeredSlotsByPhone: Map<string, { serviceId: string; date: string; time: string; expiresAt: number }[]> = new Map();

  private registerOfferedSlots(phone: string, slots: AvailabilitySlot[]) {
    if (!phone) return;
    const expiresAt = Date.now() + 20 * 60 * 1000; // válido por 20 min, mesma ordem do timeout de pagamento
    const entries = slots.map((s) => ({ serviceId: s.serviceId, date: s.date, time: s.time, expiresAt }));
    const existing = this.offeredSlotsByPhone.get(phone) || [];
    this.offeredSlotsByPhone.set(phone, [...existing.filter((e) => e.expiresAt > Date.now()), ...entries]);
  }

  private wasSlotOffered(phone: string, serviceId: string, date: string, time: string): boolean {
    const entries = (this.offeredSlotsByPhone.get(phone) || []).filter((e) => e.expiresAt > Date.now());
    return entries.some((e) => e.serviceId === serviceId && e.date === date && e.time === time);
  }

  public consultarHorarios(servico: string, dataPreferida?: string, medico?: string, telefone?: string) {
    ...
    if (telefone) {
      this.registerOfferedSlots(
        telefone,
        filteredSlots.slice(0, 8)
      );
    }
  ...
  public async criarAgendamento(
    ...
    // Trava anti-"chute": não basta o horário existir e estar livre — ele precisa ter sido
    // REALMENTE mostrado a esse paciente numa consulta recente via 'consultar_horarios_disponiveis'.
    // Sem isso, a IA poderia "acertar por coincidência" um horário válido sem nunca ter perguntado
    // ao paciente, pulando a etapa de mostrar as opções reais.
    if (!this.wasSlotOffered(telefone, serviceObj.id, date, time)) {
      const alternativas = this.slots
        .filter((s) => s.serviceId === serviceObj.id && !s.isBooked)
        .slice(0, 5)
        .map((s) => ({ data: s.date, hora: s.time }));
      ...
      const result = {
        status_agendamento: "ERRO_HORARIO_NAO_APRESENTADO",
        erro: `O horário ${date} às ${time} não foi apresentado a este paciente através de 'consultar_horarios_disponiveis'.`,
        horarios_realmente_disponiveis: alternativas,
        orientacao_importante:
          "Chame 'consultar_horarios_disponiveis' AGORA, apresente os horários reais retornados ao paciente numerados, e só chame 'criar_agendamento' depois que ele escolher um deles explicitamente.",
      };
      this.logTool("criar_agendamento", { nomePaciente, telefone, servico, dataHora, medico }, result);
      return { result, appointment: undefined };
    }
    ...
    // Cadastra ou atualiza o paciente na lista — antes o agendamento via WhatsApp
    // não deixava rastro na aba "Pacientes", só na agenda.
    const existingPatient = this.buscarPacientePorTelefone(telefone);
    if (existingPatient) {
      existingPatient.totalAppointments = (existingPatient.totalAppointments || 0) + 1;
      ...
      this.patients.unshift({
        id: `pat-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
        name: nomePaciente.trim(),
        phone: telefone.trim(),
        totalAppointments: 1,
        totalSpent: 0,
        lastVisit: date,
        generalNotes: "Cadastro iniciado via WhatsApp — completar CPF e dados na recepção.",
        clinicalHistory: [],
      });
    }
  ...
  /** Busca um paciente já cadastrado pelo telefone (normalizando dígitos), usado pela IA do WhatsApp. */
  public buscarPacientePorTelefone(telefone: string): PatientRecord | undefined {
    const cleanPhone = (telefone || "").replace(/\D/g, "");
    if (!cleanPhone) return undefined;
    return this.patients.find((p) => p.phone.replace(/\D/g, "") === cleanPhone);
  }
